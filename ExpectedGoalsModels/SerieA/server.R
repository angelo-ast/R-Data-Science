...
function(input, output, session) {
  ...
  t <- c("Atalanta", "Benevento", "Bologna", "Cagliari", "Chievo", "Crotone", "Fiorentina",
         "Genoa", "Hellas Verona", "Internazionale", "Juventus", "Lazio", "Milan", "Napoli",
         "Roma", "Sampdoria", "Sassuolo", "SPAL", "Torino", "Udinese")
  team <- sort(t)
  mat <- read.csv("data/SerieA10.csv")
  #mat <- SerieA10
  home <- mat[,1:3]
  away <- mat[,c(1:2,4)]
  ...
  ham <- ha[-c(2, 4, 6:8, 10, 12, 18, 19, 22:27),]
  hdm <- hd[-c(2, 4, 6:8, 10, 12, 18, 19, 22:27),]
  aam <- aa[-c(2, 4, 6:8, 10, 12, 18, 19, 22:27),]
  adm <- ad[-c(2, 4, 6:8, 10, 12, 18, 19, 22:27),]
  arating <- (ha$x+aa$x)/2
  drating <- (hd$x+ad$x)/2
  aratm <- (arating[2]+arating[4]+arating[6]+arating[7]+arating[8]+arating[10]+arating[12]+arating[18]+arating[19])/9
  dratm <- (drating[2]+drating[4]+drating[6]+drating[7]+drating[8]+drating[10]+drating[12]+drating[18]+drating[19])/9
  aratingm <- (ham$x+aam$x)/2
  dratingm <- (hdm$x+adm$x)/2
  b.pos <- 1
  c.pos <- 4
  d.pos <- 15
  aratingnew <- c(aratingm[1:b.pos],aratm,aratingm[(b.pos+1):c.pos],aratm,aratingm[(c.pos+1):d.pos],aratm,aratingm[(d.pos+1):length(aratingm)])
  dratingnew <- c(dratingm[1:b.pos],dratm,dratingm[(b.pos+1):c.pos],dratm,dratingm[(c.pos+1):d.pos],dratm,dratingm[(d.pos+1):length(dratingm)])

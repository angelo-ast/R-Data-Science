...
function(input, output, session) {
  ...
  t <- c("Augsburg", "Hertha BSC", "Werder Bremen", "Borussia Dortmund", "Eintracht Frankfurt", "VfB Stuttgart",
         "SC Freiburg", "Hamburger SV", "Hannover 96", "Hoffenheim", "FC Koln", "RB Leipzig", "VfL Wolfsburg",
         "Bayer Leverkusen", "Mainz 05", "Borussia Monchengladbach", "Bayern Munich", "Schalke 04")
  team <- sort(t)
  mat <- read.csv("data/Bundesliga10.csv")
  #mat <- Bundesliga10
  home <- mat[,1:3]
  away <- mat[,c(1:2,4)]
  ...
  ham <- ha[-c(1, 5, 8, 9, 11, 13, 14, 17, 20:22, 24:26, 29),]
  hdm <- hd[-c(1, 5, 8, 9, 11, 13, 14, 17, 20:22, 24:26, 29),]
  aam <- aa[-c(1, 5, 8, 9, 11, 13, 14, 17, 20:22, 24:26, 29),]
  adm <- ad[-c(1, 5, 8, 9, 11, 13, 14, 17, 20:22, 24:26, 29),]
  arating <- (ha$x+aa$x)/2
  drating <- (hd$x+ad$x)/2
  aratm <- (arating[1]+arating[8]+arating[25]+arating[9]+arating[13]+arating[11])/6
  dratm <- (drating[1]+drating[8]+drating[25]+drating[9]+drating[13]+drating[11])/6
  aratingm <- (ham$x+aam$x)/2
  dratingm <- (hdm$x+adm$x)/2
  b.pos <- 12
  aratingnew <- c(aratingm[1:b.pos],aratm,aratingm[(b.pos+1):length(aratingm)])
  dratingnew <- c(dratingm[1:b.pos],dratm,dratingm[(b.pos+1):length(dratingm)])

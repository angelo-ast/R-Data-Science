...
function(input, output, session) {
  ...
  #mat <- readxl::read_xlsx("data/EPL10.xlsx")
  mat <- EPL10
  home <- mat[,1:3]
  t <- c("Arsenal FC", "Everton FC", "Brighton & Hove Albion", "Newcastle United", "West Ham United", "Manchester United", "Chelsea FC", "Watford FC", "Tottenham Hotspur", "Manchester City",
         "Liverpool FC", "Huddersfield Town", "Stoke City", "West Bromwich Albion", "Burnley FC", "Swansea FC", "Crystal Palace", "Southampton FC", "Leicester City", "AFC Bournemouth")
  team <- sort(t)
  away <- cbind(mat[,1:2],mat[,4])
  colnames(away) <- c("home_score","away_score","away_team_id")
  ...
  ham <- ha[-c(3:7, 9, 10, 13, 15, 16, 21, 23:27, 30, 36, 37),]
  hdm <- hd[-c(3:7, 9, 10, 13, 15, 16, 21, 23:27, 30, 36, 37),]
  aam <- aa[-c(3:7, 9, 10, 13, 15, 16, 21, 23:27, 30, 36, 37),]
  adm <- ad[-c(3:7, 9, 10, 13, 15, 16, 21, 23:27, 30, 36, 37),]
  arating <- (ha$x+aa$x)/2
  drating <- (hd$x+ad$x)/2
  aratm <- (arating[1]+arating[33]+arating[23]+arating[17]+arating[8]+arating[25]+arating[9]+arating[16]+arating[12])/9
  dratm <- (drating[1]+drating[33]+drating[23]+drating[17]+drating[8]+drating[25]+drating[9]+drating[16]+drating[12])/9
  aratingm <- (ham$x+aam$x)/2
  dratingm <- (hdm$x+adm$x)/2
  b.pos <- 2
  d.pos <- 6
  aratingnew <- c(aratingm[1:b.pos],aratm,aratingm[(b.pos+1):d.pos],aratm,aratingm[(d.pos+1):length(aratingm)])
  dratingnew <- c(dratingm[1:b.pos],dratm,dratingm[(b.pos+1):d.pos],dratm,dratingm[(d.pos+1):length(dratingm)])

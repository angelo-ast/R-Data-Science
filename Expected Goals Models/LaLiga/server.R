...
function(input, output, session) {
  ...
  mat <- read.csv("data/LaLiga10.csv")
  #mat <- LaLiga10
  home <- mat[,1:3]
  away <- mat[,c(1:2,4)]
  t <- c("Alaves", "Athletic Bilbao", "Atletico Madrid", "Celta Vigo", "Deportivo La Coruna",
         "Eibar", "Espanyol", "FC Barcelona", "Getafe", "Girona", "Las Palmas", "Leganes", "Levante", "Malaga",
  ...
  ham <- ha[-c(1, 5, 8, 12, 13, 17:21, 24, 26:28, 30, 31, 34),]
  hdm <- hd[-c(1, 5, 8, 12, 13, 17:21, 24, 26:28, 30, 31, 34),]
  aam <- aa[-c(1, 5, 8, 12, 13, 17:21, 24, 26:28, 30, 31, 34),]
  adm <- ad[-c(1, 5, 8, 12, 13, 17:21, 24, 26:28, 30, 31, 34),]
  arating <- (ha$x + aa$x)/2
  drating <- (hd$x + ad$x)/2
  aratm <- (arating[5]+arating[8]+arating[12]+arating[13]+arating[17]+arating[19]+arating[24]+arating[31]+arating[34])/9
  dratm <- (drating[5]+drating[8]+drating[12]+drating[13]+drating[17]+drating[19]+drating[24]+drating[31]+drating[34])/9
  aratingm <- (ham$x + aam$x)/2
  dratingm <- (hdm$x + adm$x)/2
  c.pos <- 8
  d.pos <- 9
  aratingnew <- c(aratm, aratingm[1:c.pos],aratm,aratingm[(c.pos+1):d.pos],aratm,aratingm[(d.pos+1):length(aratingm)])
  dratingnew <- c(dratm, dratingm[1:c.pos],dratm,dratingm[(c.pos+1):d.pos],dratm,dratingm[(d.pos+1):length(dratingm)])
